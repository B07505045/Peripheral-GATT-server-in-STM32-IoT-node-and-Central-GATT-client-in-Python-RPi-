...
#define COPY_FREQUENCY_W2ST_CHAR_UUID(uuid_struct) COPY_UUID_128(uuid_struct, 0x00, 0xE0, 0x00, 0x00, 0x00, 0x02, 0x11, 0xE1, 0xAC, 0x36, 0x00, 0x02, 0xA5, 0xD5, 0xC5, 0x1B)
...
uint16_t HWServW2STHandle, EnvironmentalCharHandle, AccGyroMagCharHandle, FreqCharHandle;
...
tBleStatus Add_HWServW2ST_Service(void) {
    ...
    /* Fill the Frequency BLE Characteristc */
    COPY_FREQUENCY_W2ST_CHAR_UUID(uuid);
    BLUENRG_memcpy(&char_uuid.Char_UUID_128, uuid, 16);
    ret = aci_gatt_add_char(HWServW2STHandle, UUID_TYPE_128, char_uuid.Char_UUID_128,
			    2,
...
tBleStatus Freq_Update(int16_t freq) {
    uint8_t buff[2];
    tBleStatus ret;

    HOST_TO_LE_16(buff, freq);

    ret = aci_gatt_update_char_value(HWServW2STHandle, FreqCharHandle,
				     0, 2, buff);
    if (ret != BLE_STATUS_SUCCESS) {
	PRINTF("Error while updating Acceleration characteristic: 0x%02X\n", ret);
	return BLE_STATUS_ERROR;
...
void Read_Request_CB(uint16_t handle) {
	...
	Acc_Update(&x_axes);
    } else if (handle == FreqCharHandle + 1) {
	PRINTF("Send Freq = %d\n", freq);
...
void Write_Request_CB(evt_gatt_write_permit_req *pr) {
    ...
    if (pr->attr_handle == FreqCharHandle + 1) {
	freq = (pr->data[1] << 8) | pr->data[0];
	PRINTF("Receive Freq = %d \n", freq);
	PRINTF("data_length = %d \n", pr->data_length);

	if (pr->conn_handle != 0) {
	    ret = aci_gatt_write_response(pr->conn_handle, pr->attr_handle,
					  0x00, 0x00,
					  pr->data_length, pr->data);
	    if (ret != BLE_STATUS_SUCCESS) {
